...
global unit_pixel
unit_pixel=30/212#单位像素尺寸 mm/pixel
# 图像预处理
...
def getContours(src, img):
    ...
    for cnt in contours:
        ...
        if area > 20000:
            ...
            if objCor <= 4:
                cv2.putText(src, "width:" + str(round(width*unit_pixel))+"mm", (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.5,(0, 0, 255), 1)
                cv2.putText(src, "height:" + str(round(height*unit_pixel))+"mm", (10, 40), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
                R_area=round(width * unit_pixel)*round(height*unit_pixel)#矩形或者正方形面积
                cv2.putText(src, "area:" + str(R_area) + "mm*mm", (10, 60),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
            else:
                cv2.putText(src, "radius:" + str(round((rect[1][0]+rect[1][1])/4*unit_pixel))+"mm", (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
                C_area=round(round((rect[1][0]+rect[1][1])/4*unit_pixel)*round((rect[1][0]+rect[1][1])/4*unit_pixel)*math.pi)#圆的面积π*r*r
                cv2.putText(src, "area:" + str(C_area) + "mm*mm",(10, 40), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
            cv2.imwrite("pic/result.jpg",src)
