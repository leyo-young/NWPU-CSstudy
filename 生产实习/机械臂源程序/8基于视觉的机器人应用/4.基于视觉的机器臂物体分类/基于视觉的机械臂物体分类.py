...
class Five_Robot_Arm(QMainWindow, Ui_MainWindow):
    ...
    def object_classify(self,color,shape):
        ...
        #选择颜色分类，红蓝黄绿排列
        if self.cbx_Ispick_color.isChecked()==True and self.cbx_Ispick_shape.isChecked()==False:
            if color=="red":
                ...
                return
        #选择形状分类，左右排列
        elif self.cbx_Ispick_color.isChecked()==False and self.cbx_Ispick_shape.isChecked()==True:
            if shape=="square":
                global shape_count_square  # 正方形计数
